...
/// </summary>
public class ECKBoardView : MatrixBoardView2D
{
    ...
    protected void BuildCheckerboard()
    {
        ...
        for (int y = 0; y < matrixSize.y; y++)
        {
            for (int x = 0; x < matrixSize.x; x++)
            {
                ...
                if (isDark)
                {
                    var dropTarget = gameObject.GetComponent<ECKSquareDropTarget>();
    ...
    void PopulatePieces()
    {
        for (int y = 0; y < 3; y++)
        {
            for (int x = 0; x < matrixSize.x; x++)
            {
                ...
                var piece = pieceObject.GetComponent<ECKPieceNode>();
    ...
    {
        var pieceComponent = dragItems.dragHandlers[0].gameObject.GetComponent<ECKPieceNode>();
        var pieceLocation = pieceComponent.piece.origin;

        if (board.IsCellOccupied(location)) { return false; }
        ...
        switch (pieceComponent.isBlack)
        {
            case true:
                ...
                if (location.y - pieceLocation.y == 2 && Mathf.Abs(location.x - pieceLocation.x) == 2)
                {
                    ...
                    if (jumpedPieceNode)
                    {
                        var jumpedPieceComponent = jumpedPieceNode.GetComponent<ECKPieceNode>();

...
namespace PJ
{
    public class UnitTests_Spawner
    {
        ...
        public void TestSpawner()
        {
            ...
            var emitter = gameObject1.AddComponent<Emitter>();
            emitter.spawnAction = AddSpawn;

            emitter.spawnTable.random = fixedRandom;
            emitter.spawnTable.spawnItems.Clear();
            emitter.spawnTable.spawnItems.Add(new SpawnTable.Item(gameObject1, 0.5f));
            emitter.spawnTable.spawnItems.Add(new SpawnTable.Item(gameObject2, 0.5f));

            emitter.EmitTime = 1.0f;
            emitter.OnUpdate(new TimeSlice(0.5f));
            Assert.AreEqual(0, spawnList.Count);

            emitter.OnUpdate(new TimeSlice(1.0f));
            Assert.AreEqual(1, spawnList.Count);

            emitter.OnUpdate(new TimeSlice(.9f));
            Assert.AreEqual(1, spawnList.Count);

            emitter.OnUpdate(new TimeSlice(.3f));

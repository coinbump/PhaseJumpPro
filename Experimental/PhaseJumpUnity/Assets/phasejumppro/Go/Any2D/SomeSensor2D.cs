...
namespace PJ
{
    ...
    public abstract class SomeSensor2D : SomeSensor
    {
        ...
        /// </summary>
        protected virtual CheckOccludersResult SweepCheckOccluders(GameObject target, Angle startAngle, Angle angleSweep, bool exitIfTargetNotIn)
        {
            var startDegreeAngle = startAngle.Degrees;
            var degreeAngleSweep = angleSweep.Degrees;
            ...
            while (numSteps > 0)
            {
                var canSeeResult = CanSeeTarget(Angle.DegreesAngle(angle).ToVector2(MaxOccluderRaycastDistance), target);
        ...
        /// </summary>
        protected virtual bool CheckOccluders(Angle startAngle, Angle _maxAngleSweep, GameObject target, bool exitIfTargetNotIn)
        {
            var leftSweepCheck = SweepCheckOccluders(target, startAngle, Angle.DegreesAngle(-_maxAngleSweep.Degrees), exitIfTargetNotIn);
            if (leftSweepCheck == CheckOccludersResult.CanSeeTarget)
            {
                return false;
            }

            var rightSweepCheck = SweepCheckOccluders(target, startAngle, _maxAngleSweep, exitIfTargetNotIn);

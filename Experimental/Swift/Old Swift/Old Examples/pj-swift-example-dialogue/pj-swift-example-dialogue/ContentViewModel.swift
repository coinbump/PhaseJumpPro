...
final class ContentViewModel: ObservableObject {
    typealias Node = AcyclicGraphNode<GraphStandardEdgeModel>
        ...
        if let url = Bundle.main.url(forResource: "graph", withExtension: "json") {
            do {
                ...
                graphNodes = decodableNodes.map { decodableNode in
                    let node = Node(identifier: decodableNode.id)
                    node.tags["text"] = decodableNode.text
                    return node
                }

                for decodableNode in decodableNodes {
                    guard let fromNode = (graphNodes.first { $0.identifier == decodableNode.id }) else { continue }
                    guard let edges = decodableNode.edges else { continue }

                    for edge in edges {
                        guard let toNode = (graphNodes.first { $0.identifier == edge.id }) else { continue }

                        var tags = Tags()
                        tags["title"] = edge.title
                        fromNode.add(model: .init(identifier: edge.id, tags: tags), toNode: toNode)
                    }
                }
            }
            catch let error {

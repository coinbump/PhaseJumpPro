        ...
        .Repeat(count, [=, this](QuickBuild& qb) {
            ...
            qb.And("Matrix piece")
                ...
                .ModifyLatest<MatrixPieceHandler>([=, this](auto& handler) {
                    ...
                    auto limiter = RateLimiter::Make(animateDuration + 0.2f);
                    MatrixPieceHandler* handlerPtr = &handler;

                    RateLimiter::Type::OnFireFunc overrideFunc = [=, this](auto& limiter) {
                        GUARD(boardView)
                        auto positionMaker = QA::PositionMaker(animateDuration, EaseFuncs::inBack);

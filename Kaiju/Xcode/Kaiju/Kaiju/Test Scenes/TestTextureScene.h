        ...
        for (int i = 0; i < 1; i++) {
            ...
            if (font) {
                ...
                if (textureAtlas && !IsEmpty(textureAtlas->Textures())) {
                    ...
                    textNode.With<CycleHueEffect>().SetIsOn(true);
...
// #define TRACK_FRAME_PLAYABLE
#ifdef TRACK_FRAME_PLAYABLE
            int frameCount = 33;
            auto trackFramePlayable = MAKE<TrackFramePlayable>(33 * 0.1f);
            for (int i = 0; i < frameCount; i++) {
                trackFramePlayable->AddFrameAt(i, 0.1f * i);
            }
            //            trackFramePlayable->AddFrameAt(1, 0.2f);
            //            trackFramePlayable->AddFrameAt(2, 0.3f);
            //            trackFramePlayable->AddFrameAt(3, 0.4f);
            //            trackFramePlayable->AddFrameAt(4, 0.5f);
            //            trackFramePlayable->AddFrameAt(5, 0.6f);
            animatedTexture->framePlayable = trackFramePlayable;
#endif
...
//            auto animateHueEffect = MAKE<CycleHueEffect>();

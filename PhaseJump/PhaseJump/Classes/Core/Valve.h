...
namespace PJ
{
    ...
    /// </summary>
    class Valve : public GoCore<ValveStateType>
    {
    private:
        std::optional<float> onStateDuration = std::nullopt;    // If not null, hold on state for N seconds, then turn off
        std::optional<float> offStateDuration = std::nullopt;   // If not null, hold off state for N seconds, then turn off

    public:
        using Base = GoCore<ValveStateType>;
    ...
    public:
        float ValveState() const { return valveState; }
        void SetValveState(float value) {
            if (value == valveState) { return; }
            valveState = value;
            OnValveStateChange();
        }

 ...
 */
namespace PJ {
    class SomeCenterPolyMeshBuilder : public SomeMeshBuilder {
    public:
        Polygon polygon;

        void AddArcVertices(
            Vector2 center, Vector2 radius, float startAngle, float endAngle, float angleStep
        ) {
            ...
            while (angle < endAngle) {
                ...
                auto vertex =
                    Vector3(center.x + vector.x * radius.x, center.y + vector.y * radius.y, 0);
                polygon.Add(vertex);
        ...
        void AddArcVertices(
            Vector2 center, float radius, float startAngle, float endAngle, float angleStep
        ) {

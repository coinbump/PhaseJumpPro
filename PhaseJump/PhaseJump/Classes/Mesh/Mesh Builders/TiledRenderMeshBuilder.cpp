...
Vector2Int TiledRenderMeshBuilder::MeshSize() const {
    int numTilesX = floor(worldSize.x / tileSize.x);
    if (fmod(worldSize.x, tileSize.x)) {
        numTilesX++;
    }

    int numTilesY = floor(worldSize.y / tileSize.y);
...
RenderMesh TiledRenderMeshBuilder::BuildRenderMesh()
{
    RenderMesh result;

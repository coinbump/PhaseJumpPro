...
Mesh RoundCornersMeshBuilder::BuildMesh() {
    ...
    result += QuadMeshBuilder(leftSize).BuildMesh().OffsetBy(
        { (centerSize.x / 2.0f + leftSize.x / 2.0f) * vecLeft, 0 }
    );
    result += QuadMeshBuilder(rightSize).BuildMesh().OffsetBy(
        { (centerSize.x / 2.0f + rightSize.x / 2.0f) * vecRight, 0 }
    );
    result += QuadMeshBuilder(topSize).BuildMesh().OffsetBy(
        { 0, (centerSize.y / 2.0f + topSize.y / 2.0f) * vecUp }
    );
    result += QuadMeshBuilder(bottomSize)
                  .BuildMesh()
                  .OffsetBy({ 0, (centerSize.y / 2.0f + bottomSize.y / 2.0f) * vecDown });
    ...
    if (corners.topLeft.x > 0 && corners.topLeft.y > 0) {
        ...
        result += builder.BuildMesh().OffsetBy({ (centerSize.x / 2.0f) * vecLeft,
    ...
    if (corners.topRight.x > 0 && corners.topRight.y > 0) {
        ...
        result += builder.BuildMesh().OffsetBy({ (centerSize.x / 2.0f) * vecRight,
                                                 (centerSize.y / 2.0f) * vecUp });
    }

...
namespace PJ
{
    ...
    protected:
        ...
        SomeAnimatedEffect(float turnOnTime = 0.3f, float turnOffTime = 0.3f) : turnOnTime(turnOnTime), turnOffTime(turnOffTime) {
            valve->turnOffTimer = MAKE<TransformTimer>(turnOffTime, SomeRunner::RunType::RunOnce, MAKE<InterpolateEaseOutSquared>());
            valve->turnOnTimer = MAKE<TransformTimer>(turnOnTime, SomeRunner::RunType::RunOnce, MAKE<InterpolateEaseOutSquared>());
        }

...
namespace PJ
{
    namespace Graph
    {
        ...
        /// </summary>
        class GoStandardAcyclicNode : public GoAcyclicNode<PJ::Graph::StandardEdgeModel>
        {
        public:
            ...
            public:
                ...
                static EdgeType Input { get { return new EdgeType("input"); } }
            };
            ...
            virtual void AddOutput(String outputId, GoStandardAcyclicNode toNode, String inputId)
            {
                if (null == toNode) { return; }

                auto tags = make_shared<Tags>();
                tags[EdgeType.Output.Value] = outputId;
                tags[EdgeType.Input.Value] = inputId;

                auto outputEdgeModel = new StandardEdgeModel(outputId, EdgeType.Output.Value, 1.0f, tags);
                ...
                for (auto node : fromNodes)
                {
                    if (node.Reference.TryGetTarget(out SomeGraphNode<StandardEdgeModel> fromNode))
                    {
                        for (Edge fromEdge : fromNode.Edges)
                        {
                            ...
                            auto inputId = fromEdge.model.tags[EdgeType.Input.Value] as String;
                            if (inputId == id)
                            {
            ...
            virtual object ValueForInput<T>(String id, Tags parameters, T defaultValue)
            {
            ...
            virtual object ValueForOutput<T>(String id, Tags parameters, T defaultValue)
            {
                return defaultValue;
            }
        };

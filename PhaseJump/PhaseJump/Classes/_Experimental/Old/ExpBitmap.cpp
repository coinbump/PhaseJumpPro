...
void ExpBitmap::RenderBitmap(ExpBitmap* bitmap, int x, int y) {
    if (x < 0 || y < 0 || x >= Width() || y >= Height()) {
        PJ::Log("ERROR. ExpBitmap.RenderBitmap has bad parameters.");
        return;
    }
    ...
    if (bitmap->mPixelFormat == mPixelFormat) {
        ...
        for (int i = 0; i < bitmap->Width(); i++) {
            for (int j = 0; j < bitmap->Height(); j++) {
                ...
                float alphaFinal =
                    destFColor.alpha + sourceFColor.alpha - destFColor.alpha * sourceFColor.alpha;

                FColor finalColor;
                finalColor.red = sourceFColor.red + destFColor.red * (1.0f - sourceFColor.alpha);
                finalColor.green =
                    sourceFColor.green + destFColor.green * (1.0f - sourceFColor.alpha);
        ...
        PJ::Log("UNIMPLEMENTED. ExpBitmap.RenderBitmap doesn't support "
                "non-matching pixel formats.");
    }
}
...
char* ExpBitmap::NewVPixelLine(int x) const {
    ...
    case 2:
    default:
        PJ::Log("UNIMPLEMENTED. NewVPIxelLine for PixelSize: %d", PixelSize());
        break;
    }
...
/*
    ...
    This code is experimental. Don't use it, unless you feel brave. It may or
   may not work.
 */
...
/*
 ...
 NOTE: the value of this code is suspect, results aren't that great. Keep for
...
ExpBitmap* ExpBitmap::CreateUpsampleBicubic(int destWidth, int destHeight) {
    ...
    //         FColor    fColor =
    //         GetUpsampleBicubicDestColor(Vector2Int((float)x*xScale,
    //         (float)y*yScale), Vector2Int(destWidth, destHeight)); Color color
    //         = FColorToColor(fColor); result->SetPixelColor(x, y, color);
    //     }
    // }
    //
    // return result;
...
FColor ExpBitmap::UpsampleBicubicDestColor(Vector2Int sourcePt, Vector2Int destSize) {
    ...
    // float    dy = y - j;
    //
    ////PJ::Log("Source X, Y: %d, %d", sourcePt.x(), sourcePt.y());
    ////PJ::Log("Dest X, Y: %d, %d", i, j);
    //
    // float    red = 0.0;
    // float    green = 0.0;
    // float    blue = 0.0;
    ...
    //        green += fColor.green*BicubicWeightFoo(m-dx) *
    //        BicubicWeightFoo(dy-n); blue += fColor.blue*BicubicWeightFoo(m-dx)
    //        * BicubicWeightFoo(dy-n); alpha +=
    //        fColor.alpha*BicubicWeightFoo(m-dx) * BicubicWeightFoo(dy-n);
    //    }
    //}
    //
    // FColor    result;
...
// FUTURE: support distance searching for pixels at N distance from the fill
// zone.
uint32_t ExpBitmap::FloodFillAnyAlpha(
    Vector2Int firstPixel, FColor _fillColor, bool alter, RectInt& bounds
) {
    Color fillColor = FColorToColor(_fillColor);

    // Create an empty bitmap to check the flood fill (avoids infinite loop when
    // altering original bitmap).
    ...
    if (0 == ColorAt(firstNode.x, firstNode.y)) {
...
RectInt ExpBitmap::TrimToAnyPixels(int border) {
    ...
    if (firstLeft < 0 || firstTop < 0 || firstTop < 0 || firstBottom < 0) {
        PJ::Log("ERROR. TrimToAnyPixels. Bad bitmap.");
        return result;
    }

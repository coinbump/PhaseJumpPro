...
//	if (NULL == mTable) { return result; }
//	
//	return mTable->StringValue(key, mKeyColName, mKeyColValue, defaultValue);
//}
//
...
//}
//
//PJ_FColor SQLRow::GetFColorVal(String& key, PJ_FColor const& defaultValue) const {
//	PJ_FColor result = defaultValue;
//	if (NULL == mTable) { return result; }
//	
//	String	strVal = StringValue(key, "");
//	if (!strVal.empty()) {
...
//	String	path = fm->ApplicationDataPath("PJ_SQLRow_unit_test.sql", FileManager::PathDomain::Default);
//	fm->DeleteFile(path);
//	db.Open(path);
//	EXPECT_FALSE(db.TableExists("unit_test")) << "empty database shouldn't have tables";
//	SQLTable	table("unit_test", &db);
//	table.AddSchemaColumn("key_col", PJ_TagPtr(new PJ_IntTag(0)))->SetBoolVal(SQL_PRIMARYKEY_PNAME, true);
//	table.BuildTable();
//	

//
//  ExpTextureBuffer.cpp
//  PhaseJump
//
//  Created by Jeremy Vineyard on 4/22/23.
//
...
public:
    ...
    GLuint GetTextureID() const { return glBufferTextureId; }

    void PreRenderOrtho(bool clear);

    // OVERRIDE:
    virtual void Present() {}    // Use RenderAt2D
    virtual Vector2 Size() const { return Vector2(static_cast<float>(size.x()), static_cast<float>(size.y())); }
    virtual uint32_t PixelWidth() const { return size.x(); }
    virtual uint32_t PixelHeight() const { return size.y(); }
};
...
void GLTextureBuffer::PreRenderOrtho(bool clear)
{
    ...
    renderEngine->LoadOrtho(0, static_cast<float>(size.x()), static_cast<float>(size.y()), 0, 0.0, 1.0f);
    ...
    /*
        ...
        When rendering to a texure buffer, screen tearing can occur if glClear is called and a
        UIView is opened over the OpenGL view. The cause is uncertain.

        BEST PRACTICE: when opening a UIView, create a temporary opaque layer that blocks renders of
        OpenGL views using offscreen texture buffers, to prevent screen tears.
...
PJ_GLES2TextureBuffer::~PJ_GLES2TextureBuffer()
{
    ...
    if (mFrameBuffer != 0) {
        glDeleteFramebuffers(1, &mFrameBuffer); mFrameBuffer = 0;
    }
    if (mBufferTexture != 0) {
        glDeleteTextures(1, &mBufferTexture); mBufferTexture = 0;
    }
...
void PJ_GLES2TextureBuffer::Build(PJ_Vector2Int size)
{
    ...
    mSize = size;
    mBounds.SetSize(static_cast<float>(mSize.x()), static_cast<float>(mSize.y()));    // For drawable size queries.

    // Create and bind the frame buffer.
    glGenFramebuffers(1, &mFrameBuffer);
    //PJLog("Old Buffer: %d, Texture buffer: %d", oldBuffer, mFrameBuffer);
    renderEngine->BindFrameBuffer(mFrameBuffer);
    ...
    // IMPORTANT: without these parameters set, the texture will render as a black image.
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

    // ? GL_RGBA8 ?
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, mSize.x(), mSize.y(), 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        PJLog("Failed to make complete texture buffer object %x", glCheckFramebufferStatus(GL_FRAMEBUFFER));
    }
#endif
...
void PJ_GLES2TextureBuffer::RenderAt2D(PJ_REQuad const& v)
{
    ...
    // NOTE: in OpenGL, textures are stored "upside down", so the texture coordinates
    // for rendering the texture buffer must be adjusted accordingly.

...
namespace PJ
{
	...
	/// </summary>
	public class WeakGameObjectList : List<WeakReference<Node>>
	{
		public void Add(Node gameObject)
		{
			Add(new WeakReference<Node>(gameObject));
		}

		public void Remove(Node gameObject)
		{
			Remove(new WeakReference<Node>(gameObject));
		}
		...
		public void Refresh()
		{
			...
			foreach (WeakReference<Node> r in this)
			{
				...
				if (r.TryGetTarget(out Node gameObject))
				{
					try
					{

...
public class GraphNode<EdgeModel> where EdgeModel: Hashable
{
    public typealias Node = GraphNode<EdgeModel>
    ...
    public func evtUpdateRoot(with timeSlice: TimeSlice)
    {
        let nodes = collectGraph()
        for node in nodes
        {
            node.evtUpdate(with: timeSlice)
        }
    }

    /// Override if graph node needs to respond to time updates
    public func evtUpdate(with timeSlice: TimeSlice)
    {
    }
}
...
extension GraphNode: Equatable, Hashable {
    public func hash(into hasher: inout Hasher) {
